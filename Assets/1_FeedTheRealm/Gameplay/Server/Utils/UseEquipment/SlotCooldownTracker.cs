...
{
    /// <summary>
    /// Tracks use cooldowns for fast slots and item types (weapons and consumables) independently.
    /// Weapon and consumable cooldowns are keyed by item ID so they persist across slot changes.
    /// </summary>
    public class SlotCooldownTracker
    {
        ...
        private readonly Dictionary<string, float> weaponExpiry = new();
        private readonly Dictionary<string, int> weaponAmmo = new();
        ...
        public bool IsWeaponCoolingDown(string itemId, out float remaining)
        {
            if (weaponExpiry.TryGetValue(itemId, out float expiry))
            {
                remaining = Mathf.Max(0f, expiry - Time.time);
                return remaining > 0f;
            }
        ...
        public void RecordWeaponUsed(string itemId, float cooldown)
        {
            weaponExpiry[itemId] = Time.time + cooldown;
        }

        public int GetWeaponAmmo(string itemId, int maxAmmo)
        {
            if (!weaponAmmo.TryGetValue(itemId, out int ammo))
            {
                ammo = maxAmmo;
                weaponAmmo[itemId] = ammo;
            }
            return ammo;
        }
