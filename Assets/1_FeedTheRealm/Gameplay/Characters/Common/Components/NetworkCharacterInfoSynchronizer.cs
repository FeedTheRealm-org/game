...
public class NetworkCharacterInfoSynchronizer : NetworkBehaviour {
    ...
    public override void OnStartClient() {
        logger?.Log($"[NetworkCharacterInfoSynchronizer] OnStartClient - isLocalPlayer: {isLocalPlayer}, netId: {netId}", this);
        ...
        if (isLocalPlayer) {
            ...
            if (!string.IsNullOrEmpty(userId)) {
                ...
                logger?.Log($"[NetworkCharacterInfoSynchronizer] Local player sent UserID: {userId}", this);
            } else {
    ...
    private void CmdSendUserId(string userId) {
        syncedUserId = userId;
        logger?.Log($"[NetworkCharacterInfoSynchronizer] Server received UserID: '{userId}', netId: {netId}", this);
    }
    ...
    private void OnUserIdChanged(string oldValue, string newValue) {
        logger?.Log($"[NetworkCharacterInfoSynchronizer] OnUserIdChanged: old='{oldValue}', new='{newValue}', isLocalPlayer={isLocalPlayer}", this);
    ...
    private void LoadSpritesForRemotePlayer(string userId) {
        if (spriteLoader != null) {
            spriteLoader.UserId = userId;
            spriteLoader.StartLoadingSprites();
            logger?.Log($"[NetworkCharacterInfoSynchronizer] Remote player loading sprites for UserID: {userId}", this);
        }
    }
    ...
    private System.Collections.IEnumerator CheckSyncedUserIdDelayed() {
        ...
        if (!string.IsNullOrEmpty(syncedUserId)) {
            LoadSpritesForRemotePlayer(syncedUserId);
            logger?.Log($"[NetworkCharacterInfoSynchronizer] Remote player found synced UserID after wait: {syncedUserId}", this);
        } else {
            logger?.Log($"[NetworkCharacterInfoSynchronizer] Remote player timed out waiting for synced UserID after {timeout}s", this, Logging.LogType.Warning);
        }
    }
}
