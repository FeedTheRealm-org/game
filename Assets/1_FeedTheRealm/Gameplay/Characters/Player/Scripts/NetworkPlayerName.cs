...
public class NetworkPlayerName : NetworkBehaviour
{
    ...
    [SerializeField]
    private PlayerNameBillboard nameBillboard;
    ...
    public override void OnStartAuthority()
    {
        ...
        {
            logger?.Log(
                "[NetworkPlayerName] Session reference is not assigned on local player.",
                this,
        ...
        var name = string.IsNullOrEmpty(session.CharacterName)
            ? $"Player {netId}"
    ...
    private void ApplyName(string name)
    {
        ...
        {
            logger?.Log(
                $"[NetworkPlayerName] PlayerNameBillboard not found for netId={netId}",
                this,
                Logging.LogType.Warning
            );
        }
