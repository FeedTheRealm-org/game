...
public class SpriteLoader : MonoBehaviour
{
    ...
    [SerializeField]
    private bool loadLoggedUser = false;
    ...
    private async Task InitCharacterSpritesAsync()
    {
        ...
        var loadedCategoryIds = new HashSet<string>();

        foreach (var entry in characterInfo.category_sprites)
        {
            if (loadedCategoryIds.Contains(entry.Key))
                continue;

            loadedCategoryIds.Add(entry.Key);

            var cosmeticsResponse = await assetsService.GetSpritesByCategoryAsync(entry.Key);
            if (cosmeticsResponse?.sprites_list == null)
                continue;

            foreach (var sprite in cosmeticsResponse.sprites_list)
            {
                if (
                    sprite == null
                    || string.IsNullOrEmpty(sprite.sprite_id)
                    || string.IsNullOrEmpty(sprite.sprite_url)
                )
                {
                    continue;
                }

                if (!spriteUrlsById.ContainsKey(sprite.sprite_id))
                {
                    spriteUrlsById[sprite.sprite_id] = sprite.sprite_url;
                }
            }
        }
        ...
        foreach (var entry in characterInfo.category_sprites)
        {
            ...
            if (!spriteUrlsById.TryGetValue(entry.Value, out var spriteUrl))
            {
                ...
                {
                    logger.Log(
                        $"Failed to resolve sprite URL for sprite_id: {entry.Value}",
                        this,
                        Logging.LogType.Warning
                    );
                    continue;
