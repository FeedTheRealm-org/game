...
public class CustomNetworkManager : NetworkManager
{
    ...
    public override void OnStartServer()
    {
        ...
        {
            logger?.Log(
                "[CustomNetworkManager] WARNING: PlayerSpawnManager not found! Players will spawn at default position.",
                this,
    ...
    public override void OnServerAddPlayer(NetworkConnectionToClient conn)
    {
        ...
        logger?.Log(
            $"[CustomNetworkManager] Player spawned for connection {conn.connectionId} at position {player.transform.position}",
            this
        );
    }
    ...
    public override Transform GetStartPosition()
    {
        ...
        if (spawnManager == null)
        {
            ...
            {
                logger?.Log(
                    "[CustomNetworkManager] PlayerSpawnManager found dynamically in scene",
                    this
                );
            }
        }
        ...
        )
        {
            ...
            logger?.Log(
                $"[CustomNetworkManager] Using spawn point {spawnIndex}: {spawnPoint.position}",
                this
            );
            return spawnPoint;
        }
        ...
        logger?.Log(
            "[CustomNetworkManager] No PlayerSpawnManager available, using default spawn",
            this,
            Logging.LogType.Warning
        );
