    ...
    private IEnumerator InitializeCorrectManager()
    {
        ...
        if (isDedicatedServer)
        {
            ...
            if (serverItemsManager != null)
            {
                ...
                {
                    DebugLog(
                        $"✅ DedicatedServerItemsManager initialized with {serverItemsManager.TotalItemsLoaded} items"
                    );
                }
                else
                {
                    Debug.LogError(
                        "[ItemsManagerBootstrap] ❌ Failed to initialize DedicatedServerItemsManager!"
                    );
                }
            }
            else
            {
                Debug.LogError(
                    "[ItemsManagerBootstrap] ❌ DedicatedServerItemsManager not assigned!"
                );
            }
        }
        else
        {
            ...
            if (clientItemsManager != null)
            {
                ...
                DebugLog(
                    $"✅ ItemsManager initialized with {clientItemsManager.TotalItemsLoaded} items, {clientItemsManager.TotalSpritesLoaded} sprites"
                );
            }
            else
            {
